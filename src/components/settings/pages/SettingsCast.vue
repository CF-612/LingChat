  ...
  <MenuPage>
    ...
    <MenuItem :title="$t('settings.cast.performance')" size="small">
      ...
      <div class="flex flex-col gap-3 w-full">
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">{{ $t('settings.cast.fps') }}（{{ fps }}）</span>
          <Slider v-model="fps" :min="1" :max="30" @change="onFpsChange">1/30</Slider>
        </div>
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">{{ $t('settings.cast.quality') }}（{{ quality }}）</span>
    ...
    <MenuItem :title="$t('settings.cast.characterAdjust')" size="small">
      ...
      <div class="flex flex-col gap-3 w-full">
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">
            {{ $t('settings.cast.charScale') }}（{{ charScale.toFixed(2) }}）
          </span>
          <Slider v-model="charScale" :min="0.5" :max="2" :step="0.05" @change="onCharTuneChange">0.5/2</Slider>
        </div>
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">
            {{ $t('settings.cast.charOffsetX') }}（{{ Math.round(charOffsetX) }}px）
          </span>
          <Slider v-model="charOffsetX" :min="-400" :max="400" :step="10" @change="onCharTuneChange">-400/400</Slider>
        </div>
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">
            {{ $t('settings.cast.charOffsetY') }}（{{ Math.round(charOffsetY) }}px）
          </span>
          <Slider v-model="charOffsetY" :min="-400" :max="400" :step="10" @change="onCharTuneChange">-400/400</Slider>
        </div>
    ...
    <MenuItem :title="$t('settings.cast.dialogAdjust')" size="small">
      ...
      <div class="flex flex-col gap-3 w-full">
        <div class="w-full">
          ...
          <Slider v-model="dialogWidth" :min="30" :max="100" :step="5" @change="onDialogTuneChange">30/100</Slider>
        </div>
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">
            {{ $t('settings.cast.dialogHeight') }}（{{ Math.round(dialogHeight) }}vh）
          </span>
          <Slider v-model="dialogHeight" :min="10" :max="100" :step="5" @change="onDialogTuneChange">10/100</Slider>
        </div>
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">
            {{ $t('settings.cast.dialogFontSize') }}（{{ Math.round(dialogFontSize) }}px）
          </span>
          <Slider v-model="dialogFontSize" :min="12" :max="40" :step="1" @change="onDialogTuneChange">12/40</Slider>
        </div>
        <div class="w-full">
          <span class="block text-xs text-white/70 mb-1">
            {{ $t('settings.cast.dialogBgOpacity') }}（{{ Math.round(dialogBgOpacity) }}%）
          </span>
          <Slider v-model="dialogBgOpacity" :min="0" :max="100" :step="5" @change="onDialogTuneChange">0/100</Slider>
        </div>
    ...
    <!-- 连接信息卡 -->
    <MenuItem :title="$t('settings.cast.connection')" size="large">
      <template #header>
      ...
      <template v-if="running">
        <div class="flex flex-col gap-2 w-full">
          <div class="rounded-xl bg-black/20 border border-white/10 p-3">
            <div class="text-xs text-white/50 mb-1">{{ $t('settings.cast.streamUrlLabel') }}</div>
            <div class="cast-url">{{ streamUrl }}</div>
            <div class="text-xs text-white/50 mt-2 mb-1">{{ $t('settings.cast.pageUrlLabel') }}</div>
...
import { AlignLeft, Cast, Gauge, MessageSquareOff, MonitorPlay, Palette, PictureInPicture2, PlugZap, Scan, Wifi } from 'lucide-vue-next'
